...
const STATUS_COLOR: Record<Asset['status'], string> = palette.status;
const HIGHLIGHT = palette.status.CRITICAL;
...
export default function UAVModel({
  ...
  const matProps = {
    color,
    emissive: color,
    emissiveIntensity: wireframe ? 0.3 : 0.12,
    wireframe,
    transparent: true,
    opacity: wireframe ? 0.75 : 1,
    roughness: 0.45,
    metalness: 0.25,
  };
...
function DroneBody({
  ...
  return (
    <group>
      ...
      {/* Rotor pylons + spinning rotors (highlighted one flagged in critical red) */}
      {[
          ...
          <group key={i} position={r.pos as unknown as [number, number, number]}>
            <mesh>
              ...
              <meshStandardMaterial
                ...
                emissive={hl ? HIGHLIGHT : color}
                emissiveIntensity={hl ? 0.6 : 0.15}
            ...
            <mesh ref={rotorRefs[i]} position={[0, 0.18, 0]}>
              ...
              <meshStandardMaterial
                color={hl ? HIGHLIGHT : color}
                emissive={hl ? HIGHLIGHT : color}
                emissiveIntensity={hl ? 0.8 : 0.2}
                transparent opacity={0.85}
              />
            </mesh>
            {hl && (
              <pointLight color={HIGHLIGHT} intensity={1.5} distance={3} />
            )}
          </group>
        );
      })}
      ...
      <mesh position={[0, -0.15, 1.05]}>
        ...
        <meshStandardMaterial color={palette.foreground} emissive={palette.primary} emissiveIntensity={0.15} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group rotation={[0, 0, Math.PI / 2]}>
      ...
      <mesh position={[0, 0.4, 0]}>
        ...
        <meshStandardMaterial
          color={gyroHL ? HIGHLIGHT : color}
          emissive={gyroHL ? HIGHLIGHT : color}
          emissiveIntensity={gyroHL ? 0.7 : 0.2}
        />
      </mesh>
      {gyroHL && <pointLight color={HIGHLIGHT} intensity={1.5} distance={3} position={[0, 0.4, 0]} />}
      {/* Exhaust */}
      <mesh position={[0, -1.7, 0]}>
        <cylinderGeometry args={[0.2, 0.28, 0.25, 16]} />
        <meshStandardMaterial color={palette.foreground} emissive={palette.primary} emissiveIntensity={0.2} />
      </mesh>
    </group>
  );
}
